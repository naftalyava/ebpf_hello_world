...
// static inline int handle_ubuf(struct iov_iter *iter, struct ip_event_t *event);
...
// static inline void clear_event(struct ip_event_t *event);
// static inline int submit_event(struct ip_event_t *event);
...
static inline void clear_event(struct ip_event_t *event) {
    ...
    // We don't need to clear the payload array as we'll only read up to payload_length
}

static inline int submit_event(struct ip_event_t *event) {
    return bpf_ringbuf_output(&ip_events, event, sizeof(*event), 0);
}
